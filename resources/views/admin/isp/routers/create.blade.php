            ...
            <div class="card-body">
                <form action="{{ route('admin.isp.routers.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="ip_address" value="{{ old('ip_address') }}" class="form-control @error('ip_address') is-invalid @enderror" required>
                        @error('ip_address')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label">NAS Secret</label>
                        <input type="text" name="nas_secret" value="{{ old('nas_secret') }}" class="form-control @error('nas_secret') is-invalid @enderror" required>
                        @error('nas_secret')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Port</label>
                        <input type="number" name="port" value="{{ old('port', 8728) }}" class="form-control @error('port') is-invalid @enderror" required>
